...
def label_encoder(data: pd.DataFrame)-> Tuple[np.ndarray, np.ndarray, List[int]]:
  ...
  with open('venv/project/data/test2.pkl', 'rb') as f:
...
def save2_pkl(
  x_data: np.ndarray, y_data: np.ndarray, path: str = 'data2.pkl', per: int =0.1, random: int=0
  )-> None:
...
def training_label(data):
  ...
  if not(os.path.isfile('venv/project/data/encoder_standard2.pkl')):
    ...
    with open('venv/project/data/test2.pkl', mode='wb') as f:
      pickle.dump([
...
def main():
  #if not(os.path.isfile('venv/project/data/hospital_standard2.pkl')):
    data = equal_target(pd.read_parquet('venv/project/data/hospital.parquet'), 235677)
    x_data, y_data, columns = label_encoder(data)
    save2_pkl(x_data, y_data, 'venv/project/data/hospital_standard2.pkl')
