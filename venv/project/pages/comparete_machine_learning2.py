...
def regression_linear(
  ...
  st.markdown('### Resultado do aprendizado por regessao linear')
  if not(os.path.isfile('venv/project/data/regression_linear.pkl')):
    regression_linear = LinearRegression()
    regression_linear.fit(x_training, y_training)
    with open('venv/project/data/regresion_linear.pkl', mode='wb') as f:
      pickle.dump(regression_linear, f)
  else:
    with open('venv/project/data/regression_linear.pkl', 'rb') as f:
      regression_linear = pickle.load(f)
    prevision_regression_linear = regression_linear.predict(x_test)
    table_report(y_test, prevision_regression_linear, 'Regression Linear')
    confusion_graph(y_test, prevision_regression_linear, 'Regression Linear')
...
regression_linear(x_training,y_training,x_test,y_test)